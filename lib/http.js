...
// Handles SOAP/MTOM attached binaries and populates them in response
HttpClient.prototype.handleBinaries = function(res, body) {
  var binaries = {};
  var boundary = res.headers['content-type'].match(/boundary=([\w-]*);/i);
  
  if (boundary) {
    boundary = boundary[0];
    
    var match = body.match(/<xop\:Include href="cid\:([\w@\.-]*)"><\/xop\:Include>/gi);
  ...
  if (binaries.length > 0) {
    ...
    for (var i = 0, len = bodyParts.length; i < len; i++) {
      var part = bodyParts[i];
      var binId = part.match(/^Content-Id: <([\w\/\.@-]*)>$/m)[0];
      
      binaries[binId].mime = part.match(/^Content-Type: ([\w\/-]*)$/m)[0] || null;
      binaries[binId].data = part.match(/\n\n(.*)\n$/i)[0] || null
    }
  }
