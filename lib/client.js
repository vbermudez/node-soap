...
Client.prototype._invoke = function(method, args, location, callback, options, extraHeaders) {
  ...
  req = self.httpClient.request(location, xml, function(err, response, body) {
    ...
    // Populates response binaries as JSON Map, having as keys the binary id represented
    // in the xop:Include tag of the SOAP response. Each entry have the mime type and
    // the unparsed binary data. self.binaries = { 'cid' : { mime: 'mime/type', data: 'raw data' } }
    self.binaries = response.binaries;
